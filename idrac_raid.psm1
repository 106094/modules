function idrac_raid ([string]$para1){
    ...
    $idsetconn2=$driver.FindElement([OpenQA.Selenium.By]::Id("SataSettingsRef"))
    $idsetconn2.Click()
    
    start-sleep -s 10

    $idsatavalue=$driver.FindElement([OpenQA.Selenium.By]::Id("SataSettingsRef.EmbSata"))
    $setcount=0

    if($idsatavalue.GetAttribute("value").split(":")[1] -match $MODE){
        #Setting value same $mode , only screenshot
        screenshot -para3 nolog -para5 "original_$MODE"
    }
    else{   
        #Setting value not $mode , change setting
        screenshot -para3 nolog -para5 "original_$MODE"
        ...
        do{
            $setcount++
            start-sleep -s 5
            $idsatavalue.SendKeys($MODE)
            start-sleep -s 5
            $selected_option2 = $idsatavalue.GetAttribute("value").split(":")[1]
            if($selected_option2 -match $MODE){
                screenshot -para3 nolog -para5 "SATASetting_$($MODE)_Pass"
            }
            else{
                screenshot -para3 nolog -para5 "SATASetting_$($MODE)_Fail"
            }
       }until($selected_option2 -match $MODE -or  $setcount -gt 3)
...
Export-ModuleMember -Function idrac_raid