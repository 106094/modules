...
$mdpath=(gci -path "C:\testing_AI\modules\" -r -file |?{$_.name -match "disable_wu" -and $_.name -match "psm1"}).fullname
...
$mdpath=(gci -path "C:\testing_AI\modules\" -r -file |?{$_.name -match "initial" -and $_.name -match "psm1"}).fullname
Import-Module $mdpath -WarningAction SilentlyContinue -Global
...
if((gci $sysfd -file).count -gt 0){
$errorfoler=(gci $sysfd -file).fullname
$errorfolername=(gci $sysfd -file).name 
$lastfolder=(gci $sysfd -Directory|sort creationtime|select -Last 1).name
if(($lastfolder -$errorfolername) -eq 1){
remove-item $errorfoler -Force
rename-item (gci $sysfd -Directory|sort creationtime|select -Last 1).fullname -NewName $($errorfolername)
}
...
start-sleep -s 1
$countcb=(gci $sysfd -Directory).count-1
$countname=split-path -leaf $errorfoler
write-host "folder count $($countcb) ; error folder $($countname)" 

if(($countcb-$countname) -eq 1 ){
remove-item $errorfoler -Force
rename-item (gci $sysfd -Directory|sort creationtime|select -Last 1).fullname -NewName $($countname)
...
$cyfoldes=gci -Path $sysfd -directory -Exclude "ini" |sort {[int]$_.Name}

### compare results ###
$driverfilename1=(gci $inifd_ini\*DriverVersion.csv |Sort-Object lastwritetime |Select-Object -Last 1|?{$_.name -notmatch "all"}).FullName
$old_drivercsv=import-csv -path $driverfilename1|sort InfName
$old_sysfile= (gci -path $inifd_ini\*SystemInfo.txt|Sort-Object lastwritetime |Select-Object -Last 1).fullname
$old_sys=get-content -path $old_sysfile |select  -Skip  1  
$old_appfile=(gci $inifd_ini\*AppVersion*.csv|Sort-Object lastwritetime |Select-Object -Last 1).FullName
...
$timestart=get-date((gci $inifd_sysct\*start.jpg|Sort-Object lastwritetime |Select-Object -Last 1 ).LastWriteTime) -Format  "yyyy/M/d HH:mm:ss"
$timeend =get-date((gci $inifd_sysct\*SystemInfo.txt|Sort-Object lastwritetime |Select-Object -Last 1 ).LastWriteTime) -Format  "yyyy/M/d HH:mm:ss"
write-host " check $inifd_sysct"
##driver## 

$fd_count=$cyfolde.name
$driverfilename2=(gci $inifd_sysct\*DriverVersion.csv|Sort-Object lastwritetime |Select-Object -Last 1 |?{$_.name -notmatch "all"}).FullName
...
$old_ybct=get-content ((gci "$inifd_ini\*yellowbang.csv"|Sort-Object lastwritetime |Select-Object -Last 1).FullName)
$new_ybct=get-content ((gci "$inifd_sysct\*yellowbang.csv" |Sort-Object lastwritetime |Select-Object -Last 1 ).FullName)
...
### system ##
$new_sysfile= (gci $inifd_sysct\*SystemInfo.txt |Sort-Object lastwritetime |Select-Object -Last 1 ).fullname
...
## APP ##
$new_appfile=(gci -path $inifd_sysct\*AppVersion*.csv|Sort-Object lastwritetime |Select-Object -Last 1).fullname
...
<##
    #(gci C:\Users\MG_W251\Desktop\logs\TC-120562\6_ipmishutdown\sys -directory).FullName|?{(gci $_ -file).count -eq 0}
    (gci C:\Users\MG_W251\Desktop\logs\TC-120562\6_ipmishutdown\sys -directory).FullName|?{(gci $_ -file).count -eq 0}|remove-item -Force
$folders=((gci C:\Users\MG_W251\Desktop\logs\TC-120562\6_ipmishutdown\sys -directory -Exclude "*ini*")|Sort-Object {[int64]($_.Name)}).FullName

foreach($folder in $folders){
$folders2=((gci C:\Users\MG_W251\Desktop\logs\TC-120562\6_ipmishutdown\sys -directory -Exclude "*ini*")|Sort-Object {[int64]($_.Name)}).FullName
